var usersfuncs={
    ...
    newRow.innerHTML = `
      <td><input type="text" name="designation[]" placeholder="Enter designation" class="form-control" value=""></td>
      <td>
      ...
      <td><input type="text" name="role[]" placeholder="Enter role" class="form-control" value=""></td>
      <td><button type="button" class="btn btn-danger" onclick="usersfuncs.removeRow(this)">Delete</button></td>
    `;
  ...
  removeRow:function(button) {
   const row = button.parentElement.parentElement;
	...
	showList:function(resp,otherparams){
		...
		$(".back-to-list-button").addClass('d-none').attr('href',"roster.php#"+usersfuncs.curr_page_hash);
	...
	openRecordForEditing:function(recordid){
		...
		$("#common-processing-overlay").removeClass('d-none');
		$("#record-add-cancel-button").attr('href',"roster.php#"+usersfuncs.prev_page_hash);
	...
	showLeadDetailsWindow:function(resp,otherparams){
		...
		if(otherparams.mode=='editrecord'){
			...
			if(user_id!=''){
                ...
                //console.log(dob);
				contobj.find("form[name=addrosterform]:eq(0)").data('mode','edit-user').find('input[name=status]').attr('checked',false).end().get(0).reset();
			...
			if(user_id!=''){
				...
				$('#vu_pg_edit_btn_cont').toggleClass('d-none', !resp[1].can_edit).find('a').attr('href', `roster.php#mode=edit&recid=${user_id}`); 
	...
	handleAddUserResponse:function(resp){
		...
		if(resp.error_code==0){
			...
			$("form[name=addrosterform]").find(".error-field").removeClass('error-field').end().get(0).reset();
	...
	saveUserDetails:function(formelem){
		...
		if(res.error_fields.length>0){
			...
			alert(res.errors[0]);
			setTimeout(function(){
				$(res.error_fields[0],'#addrosterform').focus();
			},0);
			return false;
...
// 		}
	...
	openAddUserForm:function(e){
		...
		$("#user_list_container").addClass('d-none');
		$("#user_detail_view_container").addClass('d-none');
		$("#user_detail_add_edit_container").removeClass('d-none').find("#panel-heading-text").text('Add Roster').end();
		$('#msgFrm').removeClass('d-none');
		$(".back-to-list-button").removeClass('d-none');
		//$("#add_password_msg").removeClass('d-none');
		//$("#edit_password_msg").addClass('d-none');

		$("#user_detail_add_edit_container").find("#record-save-button").removeClass('d-none disabled').attr('disabled', false).find("span:eq(0)").html('Add Roster').end().end().find("#add_edit_mode").val('createUser').end().find("#add_edit_recordid").val('').end().find("#add_edit_usertype").val('').end().find("#record-add-cancel-button").data('back-to','').attr('href',"meeting.php#"+usersfuncs.prev_page_hash);
		$("form[name=addrosterform]").data('mode','add-user').find(".error-field").removeClass('error-field').end().find('input[name=status]').attr('checked',false).end().get(0).reset();
	...
	openAddMemberEditForm:function(e){
		...
		$("#user_list_container").addClass('d-none');
		$("#user_detail_view_container").addClass('d-none');
		$("#user_detail_add_edit_container").removeClass('d-none').find("#panel-heading-text").text('Edit Roster').end();
		...
		$("#user_detail_add_edit_container").find("#record-save-button").removeClass('d-none disabled').attr('disabled', false).find("span:eq(0)").html('Update Roster').end().end().find("#add_edit_mode").val('createUser').end().find("#add_edit_recordid").val('').end().find("#add_edit_usertype").val('').end().find("#record-add-cancel-button").data('back-to','').attr('href',"roster.php#"+usersfuncs.prev_page_hash);
		$("form[name=addrosterform]").data('mode','add-user').find(".error-field").removeClass('error-field').end().find('input[name=status]').attr('checked',false).end().get(0).reset();
	...
	onHashChange:function(e){
		...
		switch(hash_params.mode.toLowerCase()){
			...
			case 'edit':
							...
							if(hash_params.hasOwnProperty('recid') && hash_params.recid!=''){
								usersfuncs.openAddMemberEditForm();
