...
var login_funcs={
	...
	login:function(formelem){
		...
		var csrf_token=$(formelem).find("input[name=csrf_token]").val();
		...
		if(login_username==''){
			validation_error=1;
			validation_error_msg='Enter email ID.';
		...
		if(validation_error==1){
			...
			$.ajax({
				...
				dataType:'json',
				url:login_url,
				data:"mode=login&login_username="+encodeURIComponent(login_username)+"&login_password="+encodeURIComponent(login_password)+"&login_remember="+encodeURIComponent(login_remember)+"&referurl="+encodeURIComponent(login_referurl)+"&csrf_token="+encodeURIComponent(csrf_token),
	...
	sendRetrievePasswordRequest:function(formelem){
		...
		$("div#alert-success").addClass('d-none');
		$("div#alert-logout-success").addClass('d-none');
		var pretrieve_email=$(formelem).find("input#pretrieve_email").val();
		var csrf_token=$(formelem).find("input[name=csrf_token]").val();
		...
		$.ajax({
			...
			dataType:'json',
			url:"login.php",
			data:"mode=sendpasswordresetlink&pretrieve_email="+encodeURIComponent(pretrieve_email)+"&csrf_token="+encodeURIComponent(csrf_token),
	...
	onResetPasswordFormDisplay:function(options){
		...
		$("input#resetpswd_email_id").val(options.resp.email_id);
	...
	activateAccount:function(formelem){
		...
		$.ajax({
			...
			dataType:'json',
			url:"login.php",
			data:"mode=setpassword&resetpswd_email_id="+encodeURIComponent(resetpswd_email_id)+"&resetpswd_key="+encodeURIComponent(resetpswd_key)+"&resetpswd_password="+encodeURIComponent(resetpswd_password)+"&resetpswd_passwordre="+encodeURIComponent(resetpswd_passwordre)+"&csrf_token="+encodeURIComponent(csrf_token),
			success:function(resp,status){
	...
	setNewPassword:function(formelem){
		...
		$.ajax({
			...
			dataType:'json',
			url:"login.php",
			data:"mode=setnewpassword&resetpswd_email_id="+encodeURIComponent(resetpswd_email_id)+"&resetpswd_key="+encodeURIComponent(resetpswd_key)+"&resetpswd_password="+encodeURIComponent(resetpswd_password)+"&resetpswd_passwordre="+encodeURIComponent(resetpswd_passwordre)+"&csrf_token="+encodeURIComponent(csrf_token),
			success:function(resp,status){
