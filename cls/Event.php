...
class Event{
	...
	public function getDetails(){
		...
		$options = [];
		$options['filters'] = [
			[ 'field' => 'id', 'type' => 'EQUAL', 'value' => $this->event_id ]
		];
	...
	public static function getList($options=[]){
		...
		$fields_mapper['id']="ev.id";
		$fields_mapper['name']="ev.name";
		$fields_mapper['dsk_img']="ev.dsk_img";
		$fields_mapper['mob_img']="ev.mob_img";
		$fields_mapper['description']="ev.description";
		$fields_mapper['venue']="ev.venue";
		$fields_mapper['start_dt']="ev.start_dt";
		$fields_mapper['end_dt']="ev.end_dt";
		$fields_mapper['time_text']="ev.time_text";
		$fields_mapper['tkt_price']="ev.tkt_price";
		$fields_mapper['gst_perc']="ev.gst_perc";
		$fields_mapper['conv_fee']="ev.conv_fee";
		$fields_mapper['max_tkt_per_person']="ev.max_tkt_per_person";
		$fields_mapper['reg_start_dt']="ev.reg_start_dt";
		$fields_mapper['reg_end_dt']="ev.reg_end_dt";
		$fields_mapper['reg_active']="ev.reg_active";
		$fields_mapper['active']="ev.active";
		$fields_mapper['early_bird']="ev.early_bird";
		$fields_mapper['early_bird_tkt_price']="ev.early_bird_tkt_price";
		$fields_mapper['early_bird_end_dt']="ev.early_bird_end_dt";
		$fields_mapper['early_bird_max_cnt']="ev.early_bird_max_cnt";
			...
			foreach($options['filters'] as $filter){
				...
				switch ($filter['field']) {
					...
					case 'falls_in_period':
						...
						$dt2 = $filter['value'][1];
						if($dt1!='' && $dt2==''){
							$where_clause[] = ' '.$fields_mapper['end_dt']. ' >= :whr'.$field_counter.'_dt1 ';	
							$str_params_to_bind[':whr'.$field_counter.'_dt1']=$dt1;
						}else if($dt1=='' && $dt2!=''){
							$where_clause[] = ' '.$fields_mapper['start_dt']. ' <= :whr'.$field_counter.'_dt2 ';	
							$str_params_to_bind[':whr'.$field_counter.'_dt2']=$dt2;
						}else if($dt1!='' && $dt2!=''){
							$where_clause[] = ' ('.$fields_mapper['end_dt']. ' >= :whr'.$field_counter.'_dt1 AND '.$fields_mapper['start_dt']. ' <= :whr'.$field_counter.'_dt2  )';	
							$str_params_to_bind[':whr'.$field_counter.'_dt1']=$dt1;
							$str_params_to_bind[':whr'.$field_counter.'_dt2']=$dt2;
						}
					...
					case 'event_on_date':
						...
						break;
					case 'reg_active_on_date':
						$dt = (is_array($filter['value']))?$filter['value'][0]:$filter['value'];
						$where_clause[] = ' ( :whr'.$field_counter.'_dt between '.$fields_mapper['reg_start_dt'].' and '.$fields_mapper['reg_end_dt']. ' )';
						$str_params_to_bind[':whr'.$field_counter.'_dt']=$dt;
						break;
					...
					case 'active':
						...
						break;
					case 'curr_and_future_events_start_dt':
						$where_clause[] = '('.$fields_mapper['start_dt'].'>=:whr'.$field_counter.' OR :whr'.$field_counter.' BETWEEN '.$fields_mapper['start_dt'].' AND '.$fields_mapper['end_dt'].')';
		...
		if(array_key_exists('group_by', $options) && is_array($options['group_by'])){
			foreach ($options['group_by'] as $field) {
				if(preg_match("/^(ev)\./",$fields_mapper[$field]))
		...
		if(array_key_exists('order_by', $options) && is_array($options['order_by'])){
			foreach ($options['order_by'] as $order) {
				if(preg_match("/^(ev)\./",$fields_mapper[$order['field']])){
		...
		if(!$record_count && $order_by_clause==''){
			...
			$order_by_clause=" ORDER BY ev.start_dt DESC, ev.end_dt DESC";
		...
		$sql="SELECT $select_string from `".CONST_TBL_PREFIX."events` as ev $where_clause_string $group_by_clause $order_by_clause $limit_clause";
	...
	public function updateDetails($data){
		if($this->event_id=='')
			return false;
		return $this->saveDetails($data, $this->event_id);
	}
	...
	public function uploadImage($rec_id, $name, $tmp_name, $type){ 
		...
		$pic_size = getimagesize(CONST_EVENT_IMG_DIR_PATH.$gen_file_name);
		if(!empty($pic_size)){
			if($pic_size[0]>CONST_FIELD_META['event']['img_dim'][$type]['mw']){ // the actual width of the uploaded image exceeds the max allowed width for the $type
				$img_obj = new \eBizIndia\Img();
				if(!$img_obj->resizeImageWithADimensionFixed(CONST_EVENT_IMG_DIR_PATH . $gen_file_name, CONST_FIELD_META['event']['img_dim'][$type]['mw'], null, CONST_EVENT_IMG_DIR_PATH.$gen_file_name, 'WD')){
					unlink(CONST_EVENT_IMG_DIR_PATH.$gen_file_name);
					return false;
				}
			}
		}
